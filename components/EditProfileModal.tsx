...
import { View, Text, Button, Colors, Modal, TextField } from "react-native-ui-lib";
import { KeyboardAvoidingView, Platform } from "react-native";
import { PanGestureHandler, State } from "react-native-gesture-handler";
import Animated, { withSpring, useAnimatedStyle } from "react-native-reanimated";

const EditProfileModal = ({ translateY, visible, closeModal, field, value, onSave }) => {
                    ...
                    >
                        ...
                        <View flex-1 centerV>
                            <Text text30 style={{ fontWeight: "bold" }} center>
                                {field.includes("name") ? "Edit your name" : field.includes("phone") ? "Edit your phone number" : "Edit your email"}
                            </Text>
                            <TextField
                                marginV-20
                                padding-5
                                autoFocus
                                keyboardType={field === "Edit phone number" ? "number-pad" : "default"}
                                containerStyle={{ borderWidth: 2, borderRadius: 10, height: 50, justifyContent: "center" }}
                                placeholder={field.includes("name") ? "Enter your name" : field.includes("phone") ? "Enter your phone number" : "Enter your email"}
                                value={tempValue}
                                onChangeText={(text) => {
                                    setTempValue(text);
